...
class FakeTensorQuantFunction(Function):
    ...
    @staticmethod
    @sym_help.parse_args('v', 't', 'i', 'v', 'v')
    def symbolic(g, inputs, amax, num_bits=8, unsigned=False, narrow_range=True):
        if narrow_range:
            logging.error("narrow_range is ignored because ONNX doesn't support it.")
        amax_sequeeze = amax.squeeze()
        bound = (1 << (num_bits - 1 + int(unsigned))) - 1
        if amax.numel() > 1:
            quant_dim = list(amax.shape).index(list(amax_sequeeze.shape)[0])
        else:
            quant_dim = None
        scale = amax_sequeeze / bound
        zero_point = torch.zeros_like(amax_sequeeze, dtype=torch.int8)
        scale = g.op("Constant", value_t=scale.data)
        zero_point = g.op("Constant", value_t=zero_point.data)

        args = (scale, zero_point)
        kwargs = {}
        if amax.numel() > 1:
            kwargs['axis_i'] = quant_dim
        return g.op("DequantizeLinear", g.op("QuantizeLinear", inputs, *args, **kwargs), *args, **kwargs)
